...
// μπορεί να είναι null αν λείπει από το HTML
...
addrIn?.addEventListener("input",()=>{
  ...
  addrTimer = setTimeout(async ()=>{
    ...
    suggList.innerHTML = places.slice(0,5)
...
suggList?.addEventListener("click",e=>{
  ...
  cityInput.value=city; regionInput.value=region;
...
async function searchMemorials(){
  ...
  searchTimer = setTimeout(async ()=>{
    ...
    document.querySelectorAll(".editBtn").forEach(b=>b.addEventListener("click",loadForEdit));
  },300);
}
...
async function loadForEdit(){
  ...
  if(genealogyInput) genealogyInput.value = data.genealogy || "";

  const { data:rels } = await supabase.from("relationships")
    .select("*").eq("memorial_id",id);
...
memorialForm?.addEventListener("submit",async e=>{
  ...
  try {
    ...
    if(isNew){
      ...
      const { count } = await supabase.from("memorials")
    ...
    if(isNew){
      ...
      try {
        await supabase.storage.from("qr-codes")
          .upload(`${id}.png`, qrBlob, { contentType:'image/png' });
      } catch(err){
        if(err.status !== 409) console.error(err);
      }
    }
    const { data:pu } = supabase.storage.from("qr-codes")
      .getPublicUrl(`${id}.png`);
    qrPreview.innerHTML = `
      <h3>QR Code</h3>
