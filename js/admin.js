...
// relationships.js sets up its own listeners/UI
  ...
  document.querySelectorAll(".editBtn").forEach(btn=>{
    btn.addEventListener("click",async()=>{
      ...
      rels.forEach(r=>{
        ...
        document.querySelector("#relationshipsTable tbody").appendChild(tr);
      });
...
// ================= Submit: upsert memorial + relationships + QR =================
form.addEventListener("submit", async e=>{
  ...
  if(!rawFirst||!rawLast||!rawCity) return alert("Συμπλήρωσε Όνομα/Επώνυμο/Πόλη.");
  if(birth&&death&&new Date(birth)>new Date(death)) return alert("Η γέννηση δεν μπορεί να είναι μετά τον θάνατο.");
  ...
  try {
    ...
    const { count } = await supabase.from("memorials").select("*",{head:true,count:"exact"})
      .ilike("last_name",last_name).ilike("city",city);
    const idx = (count||0)+1;
    ...
    // handle relationships from hidden inputs
    const fd = new FormData(form);
    const relIds   = fd.getAll("relationships[][relative_id]");
    const relTypes = fd.getAll("relationships[][relation]");
    // delete old
    await supabase.from("relationships").delete().eq("memorial_id", id);
    // insert new
    if(relIds.length){
      const toInsert = relIds.map((rid,i)=>({
        memorial_id:  id,
        relative_id:  rid,
        relation_type: relTypes[i]
      }));
    ...
    const qrBlob = await (await fetch(`https://api.qrserver.com/v1/create-qr-code/?size=256x256&data=${encodeURIComponent(url)}`)).blob();
    const fn     = `${id}.png`;
